...
__global__ void permute_blocks_kernel(unsigned char *image,
    ...
    if (x >= img_dimensions.cols || y >= img_dimensions.rows) return;
    ...
    size_t block_data_offset = (size_t)current_block_linear_id * (block_size * block_size);
    
    // 7. Lookup the permutation
    // Retrieve the 1D index where the source pixel should come from.
    unsigned int src_permuted_index = permutations[block_data_offset + (local_y * block_size + local_x)];
    ...
    if (src_global_x < img_dimensions.cols && src_global_y < img_dimensions.rows) {
        image_out[y * img_dimensions.cols + x] = image[src_global_y * img_dimensions.cols + src_global_x];
    }
}
...
__global__ void permute_blocks_kernel_simple(unsigned char *image,
    ...
    unsigned int *current_permutation = ((block_idx_x + block_idx_y) & 1) 
                                        ? permutation : permutation_inverse;
    ...
    if (src_global_x < img_dimensions.cols && src_global_y < img_dimensions.rows) {
        // Scattered Read (Random access from source) -> Coalesced Write (Linear access to dest)
