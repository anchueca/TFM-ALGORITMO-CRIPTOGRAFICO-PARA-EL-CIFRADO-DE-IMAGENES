...
__host__ void encrypt_image(cv::Mat image, const std::string &password,
  ...
  const size_t num_blocks_per_row =
      image.rows / block_size + (image.rows % block_size != 0);
  const size_t num_blocks_per_col =
      image.cols / block_size + (image.cols % block_size != 0);
  ...
  const std::vector<std::vector<unsigned char>> password_segments =
      calculate_password(password, num_blocks, precision_level, rounds,
  ...
  ElementalCelularAutomata automata1(password_segments[0],
                                     image.rows * precision_level * 8, 30);
  const std::vector<ElementalCelularAutomata *> rows_automata = {&automata1};

  start = std::chrono::high_resolution_clock::now();
  unsigned int *d_permutation_rows =
      generate_automata_permutations(rows_automata, automata_steps, image.rows);
  end = std::chrono::high_resolution_clock::now();
  time = end - start;
  if (verbose)
  ...
  start = std::chrono::high_resolution_clock::now();
  unsigned int *d_permutations = generate_flow_permutations(
      password_segments[2], block_data_length, num_blocks, transition_length);
  end = std::chrono::high_resolution_clock::now();
  time = end - start;
  if (verbose)
  ...
  if (encrypt) {
    encryption_process(&d_image, &d_image_out, d_permutation_rows,
                       d_permutation_cols, d_permutations, image.cols,
                       image.rows, password_segments[3], block_size, rounds,
                       verbose);
  } else {
    start = std::chrono::high_resolution_clock::now();
    ...
    unencryption_process(&d_image, &d_image_out, d_permutation_rows,
                         d_permutation_cols, d_permutations, image.cols,
                         image.rows, password_segments[3], block_size, rounds);
  }
...
void encryption_process(unsigned char **d_image, unsigned char **d_image_out,
                        unsigned int *d_permutation_rows,
                        unsigned int *d_permutation_cols,
                        unsigned int *d_permutation_blocks, size_t cols,
                        size_t rows, std::vector<unsigned char> flow_seeds,
  ...
  for (size_t i = 0; i < rounds; i++) {
    ...
    for (size_t j = 0; j < 2; j++) { // Each round two permutations
      ...
      rows_and_columns_permutation(*d_image, *d_image_out, d_permutation_rows,
                                   d_permutation_cols, cols, rows, false);
      ...
      // Block
      start = std::chrono::high_resolution_clock::now();
      block_phase_permutation(*d_image, *d_image_out, d_permutation_blocks,
...
void unencryption_process(unsigned char **d_image, unsigned char **d_image_out,
                          unsigned int *d_permutation_rows,
                          unsigned int *d_permutation_cols,
                          unsigned int *d_permutation_blocks, size_t cols,
                          size_t rows, std::vector<unsigned char> flow_seeds,
    ...
    *d_image_out = temp;
    ...
    for (size_t j = 0; j < 2; j++) { // Each round two permutations
      ...
      *d_image_out = temp;
      ...
      // Rows and columns
      rows_and_columns_permutation(*d_image, *d_image_out, d_permutation_rows,
                                   d_permutation_cols, cols, rows, true);
    }
  }
}