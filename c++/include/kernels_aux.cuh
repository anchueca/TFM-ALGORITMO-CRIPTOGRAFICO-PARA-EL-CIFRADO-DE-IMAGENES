...
template<typename T>
/**
 * @brief Device-side insertion sort that sorts a slice of chaotic values and their corresponding indices.
 *
 ...
 *
 * Each thread (or logical index determined by the grid) handles a separate block
 * segment of length block_length. It calls the device insertion sort to reorder
 * the chaotic values and indices in-place.
 *
...
__global__ void sort_indices_by_chaotic_values_global(
    ...
    if (idx >= (int)num_blocks) return;
    int base_idx = idx * (int)block_length;

    sort_indices_by_chaotic_values<T>(base_idx, d_chaotic_values, indices, (int)block_length);
}
 ...
 * @param inverses Output array for the inverted permutations on the GPU (flattened).
