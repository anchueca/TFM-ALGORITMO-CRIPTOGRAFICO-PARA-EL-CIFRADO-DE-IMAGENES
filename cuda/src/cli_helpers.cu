...
bool parse_arguments(int argc, char **argv, AppConfig &config) {
  ...
  try {
    config.params.num_blocks_permutations = 1;
...
bool handle_output(OutputMode mode, const string &output_arg, const cv::Mat &image, bool verbose) {
  if (image.empty()) {
    cerr << "[ERROR] Resulting image is empty. Encryption/Decryption failed." << endl;
    return false;
  }
  ...
  switch (mode) {
    case OutputMode::FILE_SAVE: {
      ...
      if (verbose) std::cout << "Image saved successfully to " << output_arg << std::endl;
      break;
    }
    case OutputMode::DISPLAY_WINDOW: {
      try {
        ...
        if (verbose) std::cout << "Displaying image. Press any key to close..." << std::endl;
        cv::waitKey(0);
      } catch (const cv::Exception &e) {
