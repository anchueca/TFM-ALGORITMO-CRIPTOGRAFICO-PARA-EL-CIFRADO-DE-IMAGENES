...
def generate_password(
    ...
    hash = get_Hash(
        initial_password, length_bytes
    )  # Asumiendo que get_Hash está definida
...
def encrypt_image(image, model, password, rounds, show=0):
    ...
    color_depth = 8
    row_password, column_password, block_password, flow_password = generate_password(
        password, num_blocks, rows, columns, precission_level, color_depth, rounds
    )
    ...
    for round_number in range(rounds):
        ...
        block_permutations = [
            generate_partiton(model, block_password[i], block_data_lenght)
        ...
        if round_number < rounds - 1:  # The last rounds do not flow encrypt
            ...
            for n in range(4):
                image, actual_flow_password = flow_encrypt_image(
                    image, model, actual_flow_password
                )
...
def unencrypt_image(image, model, password, rounds, show=0):
    ...
    permutation_columns = invert_permutation(
        generate_partition_from_automata(column_password, image.shape[1])
    )
    permutation_rows = invert_permutation(
        generate_partition_from_automata(row_password, image.shape[0])
    )
    ...
    for round_number in range(rounds):
        ...
        bloock_permutations = [
            invert_permutation(permitation) for permitation in bloock_permutations
        ]  # all blocks have the same size
...
def permute_block_matrix(block_list, permutation_list):
    ...
    """
    permuted_blocks = [
        [permute_block(block, permutation) for block in block_row]
        for block_row, permutation in zip(block_list, permutation_list)
    ]
...
def permute_block(block, permutation):
    ...
    for i, idx in enumerate(permutation):
        ...
        permuted_block[h_normalized][w_normalized] = block[h_permutation_normalized][
            w_permutation_normalized
        ]
...
def compose_image_from_blocks(blocks, image_shape, num_blocks=64):
    ...
    reconstructed_image = (
        np.empty((x, y), dtype=np.uint8)
        if len(image_shape) == 2
        else np.zeros((x, y, 3), dtype=np.uint8)
    )
...
def main():
    ...
    encrypted_image = encrypt_image(image,uno, args.password, args.rounds, args.show)
    cProfile.runctx("encrypt_image(image, password, rounds, show)",globals(),locals(),"encrypt.prof",)
    end_time = time.time()
    print(f"Time: {end_time - start_time} s")

    start_time = time.time()
    unencrypted_image = unencrypt_image(encrypted_image,uno, password, rounds, show)
