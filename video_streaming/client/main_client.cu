 ...
 *
 * Protocol:
 *   stdin  → JSON commands: {"action":"connect","host":"...","port":N,"password":"..."}
 *                           {"action":"disconnect"}
    ...
    while (g_running) {
        ...
        while (g_running && !g_request_disconnect) {
            ...
            if (!recv_all(sock, pixel_data.data(), hdr.data_size)) {
                std::cerr << "[CLIENT] Failed to receive frame data for ID " << hdr.frame_id << "\n";
                break;
            }
            ...
            try {
                ...
                if (!decryptor) {
                    decryptor = new VideoEncryptor(password,
                                                  hdr.width, hdr.height,
                                                  hdr.channels, false);
                    std::cerr << "[CLIENT] Decryptor initialized for "
                              << hdr.width << "x" << hdr.height << " (" << hdr.channels << " ch)\n";
                }

                int padded_dim = decryptor->getPaddedWidth();
                if (hdr.data_size != (uint32_t)(padded_dim * padded_dim)) {
                    std::cerr << "[CLIENT] Size mismatch: expected " << (padded_dim*padded_dim)
                              << " but got " << hdr.data_size << "\n";
                ...
                cv::Mat decrypted = decryptor->processFrame(encrypted.clone(), &frame_hash);
                ...
                if (elapsed >= 1.0) {
                    ...
                    fps_count = 0;
                    fps_start = now;
                    std::cerr << "[CLIENT] Decrypting at " << g_fps << " FPS (ID: " << g_dec_frame_id << ")\n";
                }
            } catch (const std::exception& e) {
                std::cerr << "[CLIENT] Error processing frame " << hdr.frame_id << ": " << e.what() << "\n";
...
void mjpeg_client_handler(int client_fd) {
    ...
    while (g_running) {
        ...
        std::string part_header =
            ...
            "Content-Length: " + std::to_string(jpeg_to_send.size()) + "\r\n\r\n";

        if (send(client_fd, part_header.c_str(), part_header.size(), MSG_NOSIGNAL) <= 0) break;
        if (send(client_fd, jpeg_to_send.data(), jpeg_to_send.size(), MSG_NOSIGNAL) <= 0) break;
...
void mjpeg_server_thread(int port) {
    ...
    if (bind(server_fd, (sockaddr*)&addr, sizeof(addr)) < 0) {
        perror("bind MJPEG output"); close(server_fd); return;
    }
    listen(server_fd, 5);
    std::cerr << "[MJPEG-OUT] Decrypted stream at http://localhost:"
              << port << "/stream\n";
...
int main(int argc, char** argv) {
    ...
    int mjpeg_port = 9090;
    if (argc > 1) mjpeg_port = std::stoi(argv[1]);
