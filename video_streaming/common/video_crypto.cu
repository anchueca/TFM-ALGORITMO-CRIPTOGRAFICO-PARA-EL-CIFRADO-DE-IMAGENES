...
VideoEncryptor::VideoEncryptor(const std::string& password, int width,
    ...
    password_segments_ = calculate_password(password, img_dimensions_,
                                            false, false);

    std::cerr << "[VideoEncryptor] Initialized: " << width << "x" << height
              << " ch=" << channels
              << " padded=" << padded_dim_ << "x" << padded_dim_
...
cv::Mat VideoEncryptor::processFrame(const cv::Mat& frame, uint16_t* recovery_info) {
    cv::Mat processed;
    ...
    if (encrypt_) {
        ...
        for (unsigned char b : password_segments_[3]) {
            for (int i = 0; i < 8; ++i) stego_key.push_back((b >> i) & 1);
        }
        std::vector<bool> msg;
        for (int i = 0; i < 16; ++i) msg.push_back((params_.image_hash >> i) & 1);
        
        std::vector<bool> R = embed_message_caos(processed, msg, stego_key);
        
        if (recovery_info) {
            uint16_t r_val = 0;
            for (size_t i = 0; i < 16 && i < R.size(); ++i) if (R[i]) r_val |= (1 << i);
            *recovery_info = r_val;
        }
        return processed;
        ...
        if (recovery_info) {
            ...
            for (int i = 0; i < 16; ++i) r_bits.push_back((*recovery_info >> i) & 1);
            
            std::vector<bool> msg = extract_message_caos(processed, r_bits, stego_key);
            
            uint16_t h_val = 0;
            for (size_t i = 0; i < 16 && i < msg.size(); ++i) if (msg[i]) h_val |= (1 << i);
