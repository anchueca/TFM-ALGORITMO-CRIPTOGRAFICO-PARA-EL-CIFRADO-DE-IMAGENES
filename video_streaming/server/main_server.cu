 ...
 *   - TCP raw protocol (port --port)        → custom encrypted client
 *   - HTTP MJPEG      (port --mjpeg-port)  → VLC preview (encrypted noise)
 *
 */
...
static int g_actual_w = 0;
static int g_actual_h = 0;
static int g_padded_w = 0;
static int g_padded_h = 0;
...
bool parse_args(int argc, char **argv, ServerConfig &cfg) {
  for (int i = 1; i < argc; i++) {
    ...
    if (arg == "--password" && i + 1 < argc) {
    ...
    else if (arg == "--mjpeg-port" && i + 1 < argc) {
      cfg.mjpeg_port = std::stoi(argv[++i]);
    }

    else if (arg == "--device" && i + 1 < argc) {
      cfg.device = std::stoi(argv[++i]);
    }
      ...
      std::cout << "Usage: video_server --password <KEY> [options]\n"
                << "  --port <N>         TCP port (default 8554)\n"
                << "  --mjpeg-port <N>   MJPEG port (default 8555)\n"
                << "  --device <N>       Webcam device index\n"
                << "  --resolution WxH   Capture resolution\n";
...
void tcp_accept_thread(int port) {
  ...
  while (g_running) {
    ...
    std::cerr << "[TCP] Client connected: " << client_ip << "\n";
...
void tcp_broadcast_thread() {
  ...
  while (g_running) {
...

    {
      ...
      // clone only ONCE outside global storage
    ...
    for (int fd : g_tcp_clients) {
      ...
      bool ok = send_frame(fd, aw, ah, 3, padded_size, frame_to_send.data,
                           frame_id, image_hash);
...
void mjpeg_client_handler(int client_fd) {
  ...
  while (g_running) {
    ...
    if (send(client_fd, jpeg_buf.data(), jpeg_buf.size(), MSG_NOSIGNAL) <= 0)
      break;
...
void mjpeg_server_thread(int port) {
  ...
  std::cerr << "[MJPEG] Stream available at: "
            << "http://0.0.0.0:" << port << "/\n";
...
int main(int argc, char **argv) {
  ...
  if (actual_w != cfg.width || actual_h != cfg.height) {

    std::cerr << "[WARN] Requested resolution " << cfg.width << "x"
              << cfg.height << " not supported.\n"
              << "[WARN] Using " << actual_w << "x" << actual_h << "\n";
  }
  ...
  std::cerr << "\n═══════════════════════════════════════════════\n"
            ...
            << "  TCP:    " << cfg.port << "\n"
            << "  MJPEG:  http://0.0.0.0:" << cfg.mjpeg_port << "/\n"
            << "═══════════════════════════════════════════════\n\n";
  ...
  while (g_running) {
    ...
    // publish latest frame
    {
      ...
      g_latest_encrypted = std::move(encrypted);
    ...
    if (elapsed >= 2.0) {
      ...
      std::cerr << "[SERVER] FPS: " << fps << " | Frames: " << frame_counter
                << "\r" << std::flush;
