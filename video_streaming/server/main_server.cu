...
bool parse_args(int argc, char **argv, ServerConfig &cfg) {
  for (int i = 1; i < argc; i++) {
    ...
    if (arg == "--password" && i + 1 < argc)
      ...
      std::cout
          << "Usage: video_server --password <KEY> [options]\n"
...
void tcp_broadcast_thread() {
  ...
  while (g_running) {
    ...
    for (int fd : g_tcp_clients) {
      ...
      if (send_frame(fd, aw, ah, 3, padded_size, frame_to_send.data, frame_id,
                     image_hash)) {
        alive.push_back(fd);
      } else {
...
void mjpeg_server_thread(int port) {
  ...
  std::cerr << "[MJPEG] VLC stream available at http://0.0.0.0:" << port << "/"
            << std::endl;
